...
def get_conf(param, default_value=None):
    if os.getenv(param):
        ...
        out = subprocess.check_output(["snapctl", "get", snap_param]).decode("utf-8").strip()
        if out:
            return out
        else:
        ...
        for line in req.text.split("\n"):
            ...
            if fields[0] in COLLECT:
                ...
                field_mappings = [
                    ...
                    "eresp","wretr","wredis","status","weight","act",
                    "bck","chkfail","chkdown","lastchg","downtime",
                    "qlimit","pid","iid","sid","throttle","lbtot",
                    "tracked","type","rate","rate_lim","rate_max",
                    "check_status","check_code","check_duration",
                    "hrsp_1xx","hrsp_2xx","hrsp_3xx","hrsp_4xx",
                    "hrsp_5xx","hrsp_other","hanafail","req_rate",
                    "req_rate_max","req_tot","cli_abrt","srv_abrt",
                    "comp_in","comp_out","comp_byp","comp_rsp",
                    "lastsess","last_chk","last_agt","qtime",
                    "ctime","rtime","ttime"]
